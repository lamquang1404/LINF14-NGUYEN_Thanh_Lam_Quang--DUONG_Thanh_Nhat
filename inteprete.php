...
<head>
  ...
  <style>
    ...
    #header { background-image: url("instrument.jpg"); height: 400px;margin-bottom: 50px;}
...
<div>
<div id="header">
  <h2>TOUS LES INTERPRETES</h2>
</div>
<form id="f" method="get" action="inteprete.php">
   <input type="text" name="interprete" placeholder="Recherche par nom et prenom d'interpretes"><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-search"></span></button></input> 
</form>
<?php
      ...
      if (!isset($_GET['interprete']))
        $query = "select DISTINCT Musicien.Nom_Musicien, Musicien.Prénom_Musicien, Musicien.Année_Naissance, Musicien.Année_Mort, Musicien.Code_Musicien
        ...
        $query = "select DISTINCT Musicien.Nom_Musicien, Musicien.Prénom_Musicien, Musicien.Année_Naissance, Musicien.Année_Mort, Musicien.Code_Musicien
                  FROM Album INNER JOIN (Disque INNER JOIN ((Enregistrement INNER JOIN (Musicien INNER JOIN Interpréter ON Musicien.Code_Musicien = Interpréter.Code_Musicien) ON Enregistrement.Code_Morceau = Interpréter.Code_Morceau) INNER JOIN Composition_Disque ON Enregistrement.Code_Morceau = Composition_Disque.Code_Morceau) ON Disque.Code_Disque = Composition_Disque.Code_Disque) ON Album.Code_Album = Disque.Code_Album
                  WHERE Musicien.Nom_Musicien LIKE '%" . $_GET['interprete'] . "%' OR Musicien.Prénom_Musicien LIKE '%" . $_GET['interprete'] . "%'
                  ORDER BY Musicien.Nom_Musicien";
      echo 
      "<table class=\"table table-bordered\">";
        echo"<tr><th>NOM INTERPRÈTE</th><th>PRENOM INTERPRÈTE</th><th>ANNNEE NAISSANCE</th><th>ANNNEE MORT</th><th> PHOTO </th><th> ALBUMS CONCERNENT</th>";
        foreach ($pdo->query($query) as $row) {
          ...
          echo "<td> <a href=\"album-interprete.php?code=" .$row['Code_Musicien']. "\"> Albums Concernent </a></td>";
          echo "</tr>";
        }
      echo 
      "</table>";
